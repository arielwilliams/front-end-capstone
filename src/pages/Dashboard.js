...
const Dashboard = ({ list, user, setListData }) => {
  if (Object.keys(user).length > 0) {
    return (
        <div className="relative bg-cover bg-no-repeat bg-center min-h-[100vh] py-4 after:opacity-[.30]" style={{ backgroundImage: `url('/pasta.jpg')` }}>
          <section className="rounded p-4 w-full border-black bg-white">
            ...
            <div className="bg-white rounded p-4">
              <h3 className="font-bold text-2xl ml-6 text-green-900 w-full flex justify-center">
                {user.given_name}'s Lists
              </h3>
            </div>
      
            {/* Move the AllLists component here */}
