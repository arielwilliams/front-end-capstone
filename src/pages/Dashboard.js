...
const Dashboard = ({ list, user, setListData }) => {
  if (Object.keys(user).length > 0) {
    return (
      <div className="relative bg-cover bg-no-repeat bg-center min-h-[100vh] py-4" style={{ backgroundImage: `url('/dashboard_image.jpg')` }}>
        <h1 className="text-center font-semibold text-5xl m-6 text-white">
          Welcome to your Dashboard!
        </h1>
        ...
        <h2 className="text-left font-bold text-2xl ml-6 text-white">
          {user.given_name}'s Lists
        </h2>
        <AllLists list={list} setListData={setListData} />
      </div>
