  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="w-full flex justify-center"> {/* Center the button */}
        <button className="bg-emerald-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded" onClick={getRandomRestaurant}>
    ...
    <ul className="grid grid-cols-5 gap-4 p-4 justify-content: start;"> {/* Add space between each list item */}
      {list.map((restaurant) => (
        <li key={restaurant.id} className="flex justify-center">
          <div className="restaurant-card-container bg-white border rounded p-4"> {/* Container for each RestaurantCard */}
            <RestaurantCard
